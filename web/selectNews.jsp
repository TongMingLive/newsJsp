...
<%
    ...
    list = newsDao.selectNewsByKey(key);
  }
%>

<div style="margin: 20px;text-align: center" class="row">
  <h5>根据你的关键字“<%=key%>”搜索结果</h5>
  <div class="col-md-10">
    <%if (list.size() == 0){%>
      <h6>暂无该关键字的搜索内容</h6>
    <%}else {
    ...
    <h6>
      <a href="newsread.jsp?nid=<%=list.get(i).getNewsid()%>" title="<%=list.get(i).getNewstitle()%>"><%=list.get(i).getNewstitle()%></a>
